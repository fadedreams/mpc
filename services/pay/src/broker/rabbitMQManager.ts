...
import SellerService from '@pay/pay/services/sellerService';
...
export class RabbitMQManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.log.info('pay server connected to queue successfully... ', this.rabbitmqEndpoint);
    } catch (error) {
      this.log.log(`payService error initialize() method: ${this.rabbitmqEndpoint}`, error);
    }
  }
  ...
  async consumeEmailMessages(channel: Channel, exchangeName: string, routingKey: string, queueName: string, template: string): Promise<void> {
    try {
      ...
      this.log.log('error', `payService EmailConsumer consumeEmailMessages() method error: ${error}`);
    }
  }
