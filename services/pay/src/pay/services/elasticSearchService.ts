...
import { ISellerDocument } from "@pay/dto/seller.d";
...
export class ElasticSearchService {
  ...
  constructor() {
    ...
    this.log = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'payElasticSearchServer', 'debug');
  ...
  async checkConnection(): Promise<void> {
    ...
    while (!isConnected) {
      try {
        ...
        this.log.info(`payService Elasticsearch health status - ${health.status}`);
        isConnected = true;
      } catch (error) {
        this.log.error(`Connection to Elasticsearch failed. Retrying... ${config.ELASTIC_SEARCH_URL}`);
        this.log.log('error', 'payService checkConnection() method:', error);
      }
    }
  }
