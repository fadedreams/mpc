...
class DatabaseConnector {
  ...
  private constructor() {
    this.log = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'orderDatabaseServer', 'debug');
  }
  ...
  public async connect(): Promise<void> {
    try {
      ...
      this.log.info('order service successfully connected to the database.');
      console.log('order service successfully connected to the database.');
    } catch (error) {
