...
export class RabbitMQManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.log.info('order server connected to queue successfully... ', this.rabbitmqEndpoint);
    } catch (error) {
      this.log.log(`orderService error initialize() method: ${this.rabbitmqEndpoint}`, error);
    }
  }
  ...
  async consumeEmailMessages(channel: Channel, exchangeName: string, routingKey: string, queueName: string, template: string): Promise<void> {
    try {
      ...
      this.log.log('error', `orderService EmailConsumer consumeEmailMessages() method error: ${error}`);
    }
  }
