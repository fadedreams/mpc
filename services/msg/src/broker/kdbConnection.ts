...
export class KeyDBConnection {
    ...
    constructor() {
        this.log = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'msgKeyDBConnection', 'debug');
    ...
    private cacheError(): void {
        this.client.on('error', (error: unknown) => {
            this.log.error('msgService keydbConnect() method error:', error);
        });
    }
    ...
    public async connect(): Promise<void> {
        ...
        try {
            ...
            this.log.info(`msgService KeyDB Connection: ${await this.client.ping()}`);
        } catch (error) {
            this.log.error('msgService keydbConnect() method error:', error);
        }
    }
