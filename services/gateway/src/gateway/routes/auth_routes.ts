...
import { gatewayCacheInstance as gatewayCache } from '@gateway/broker/gatewayCache';
import axios, { AxiosResponse } from 'axios';
export function initAuthRoutes(socketIO: Server): Router {
  ...
  // router.get('/auth/logged-in-user', authMiddleware.verifyUser, async (req: Request, res: Response) => {
  router.get('/auth/logged-in-user', async (req: Request, res: Response) => {
    // console.log(' in /auth/logged-in-user ', socketIO);
    // const response: string[] = await gatewayCache.getLoggedInUserFromCache('loggedInUsers');
    socketIO.emit('online', "test");
    // socketIO.emit('online', response);
    // console.log('Emitted "online" event with data:', response);
