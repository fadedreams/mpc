// services/review/src/broker/redisConnection.ts
...
export class RedisConnection {
  ...
  constructor() {
    this.log = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'reviewRedisConnection', 'debug');
        ...
        socket: {
          ...
          reconnectStrategy: (retries) => {
            if (retries > MAX_RECONNECT_ATTEMPTS) {
              this.log.error(
                `reviewService Redis: exceeded ${MAX_RECONNECT_ATTEMPTS} reconnect attempts, giving up`
              );
            ...
            this.log.warn(`reviewService Redis: reconnect attempt ${retries}, retrying in ${delay}ms`);
            return delay;
          },
        },
  ...
  private cacheError(): void {
    this.client.on('error', (error: unknown) => {
      this.log.error('reviewService redisConnect() method error:', error);
    });
  }
  ...
  private handleLifecycleEvents(): void {
    ...
    this.client.on('reconnecting', () => {
      ...
      this.log.warn('reviewService Redis: connection lost, reconnecting via built-in strategy...');
    });

    this.client.on('ready', () => {
      this.isConnected = true;
      this.log.info('reviewService Redis: connection ready');
    });

    this.client.on('end', () => {
      this.isConnected = false;
      this.log.warn('reviewService Redis: connection ended');
    });
  }
  ...
  public async connect(): Promise<void> {
    ...
    this.connectingPromise = (async () => {
      ...
      try {
        ...
        this.isConnected = true;
        // NOTE: original file said "ItemService Redis Connection" here —
        // that was a copy-paste bug from another service. Fixed to reviewService.
        this.log.info(`reviewService Redis Connection: ${pong}`);
      } catch (error) {
        // NOTE: original file said "ItemService redisConnect()..." here —
        // same copy-paste bug, fixed to reviewService.
        this.log.error('reviewService redisConnect() method error:', error);
