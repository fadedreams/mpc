...
export class KeyDBConnection {
    ...
    constructor() {
        this.log = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'reviewKeyDBConnection', 'debug');
    ...
    private cacheError(): void {
        this.client.on('error', (error: unknown) => {
            this.log.error('reviewService keydbConnect() method error:', error);
        });
    }
    ...
    public async connect(): Promise<void> {
        ...
        try {
            ...
            this.log.info(`ReviewService KeyDB Connection: ${await this.client.ping()}`);
        } catch (error) {
            this.log.error('ReviewService keydbConnect() method error:', error);
        }
    }
